\documentclass[a4paper,openright,twoside,11pt]{thesis} %titlepage,openright,twoside,

%--------------------PREAMBLE----------------------------------------
\include{preamble}
\include{newcommands}

%---------Bibliography--------------------------------

\bibliography{Bibliography_papers, Thesis_bib, McMillan.bib}  %,Bibliography.bib Bibs\Bibliography_papers.bib,
\graphicspath{{./ThesisFigs/}}

%-------------Titel informatie----------------------
%Wordt in make title gegenereerd
%---------Begin Document----------------------------
\begin{document}
\cleardoublepage%
\maketitle%
%------------------Foreword-------------------------
\pagenumbering{roman}                         %
%\include{foreword/latex/Foreword}            %
%\addcontentsline{toc}{chapter}{Foreword}     %
%--------Abstract-----------------------------------
%\include{abstract/latex/abstract}
%\addcontentsline{toc}{chapter}{Abstract}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%------------Lists----------------------------------------------------------------

\renewcommand{\contentsname}{Table of Contents}  % Original name = Contents
\begin{spacing}{1.2}  % Environment for 1.2 line spacing for contents and lists   |
\tableofcontents%
\printglossaries %
\listoffigures %\addcontentsline{toc}{chapter}{List of Figures}
%\listoftables %\addcontentsline{toc}{chapter}{List of Tables}
\cleardoublepage%                                                                 |
\end{spacing}%                                                                    |
%----------------------end--------------------------------------------------------

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Arabic numbering after this               %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage\thispagestyle{empty}\cleardoublepage%
\pagenumbering{arabic} \setcounter{page}{1}%

%------------Main Matter----------------------------


%\include{intro}
%\include{Lamy_bio}
\include{Literature_Review}
%\include{WTO_1}
%\include{WTO_Roles}


\setcounter{tocdepth}{1}%
%\begin{appendices}
%\include{appendices/latex/}
%\include{appendices/latex/}
%\end{appendices}


%\nocite{*} 
\addcontentsline{toc}{chapter}{Bibliography}

\printbibliography %

\end{document}

